...
import { Trophy, Users, Gauge, Filter, CalendarDays, Download, ShieldCheck, Search, BarChart3, Database, RefreshCw, ClipboardCheck, LayoutDashboard, Lock, LogOut, Pencil, Trash2, Save, X, BadgeCheck } from "lucide-react";
...
function exportToCsv(rows) {
  ...
  const csvRows = rows.map((item) => [item.race,item.firstName,item.lastName,item.kartNumber,item.teamName,item.kartClass,item.status,item.email || "",item.createdAt ? new Date(item.createdAt).toLocaleString("de-DE") : ""]);
  const content = [csvHeader, ...csvRows].map((row) => row.map((cell) => `"${String(cell).replace(/"/g, '""')}"`).join(";")).join("\n");
...
export default function App() {
  ...
  useEffect(() => { loadRegistrations(); restoreSession(); }, []);
  ...
  async function submitRegistration(e) {
    ...
    if (!validEmail) return setFormError("Bitte gib eine gültige E-Mail-Adresse ein.");
  ...
  async function saveEdit() {
    ...
    const supabase = await getSupabaseClient();
    if (!supabase) {
      setRegistrations((prev) => prev.map((item) => (item.id === editingId ? { ...editForm } : item)));
      setEditingId(null);
      setEditForm(null);
      setAdminNotice("Demo-Modus: Starter aktualisiert.");
      return;
    }
    const { error } = await supabase.from("registrations").update({
      race: editForm.race, first_name: editForm.firstName, last_name: editForm.lastName, email: editForm.email,
      kart_number: editForm.kartNumber, team_name: editForm.teamName, kart_class: editForm.kartClass, status: editForm.status,
    }).eq("id", editingId);
    if (error) return setAdminNotice(error.message);
    await loadRegistrations();
    setEditingId(null);
    setEditForm(null);
    setAdminNotice("Starter erfolgreich aktualisiert.");
  }
  async function deleteStarter(id) {
    const confirmed = window.confirm("Starter wirklich löschen?");
    if (!confirmed) return;
    const supabase = await getSupabaseClient();
    if (!supabase) {
      setRegistrations((prev) => prev.filter((item) => item.id !== id));
      setAdminNotice("Demo-Modus: Starter gelöscht.");
      return;
    }
    const { error } = await supabase.from("registrations").delete().eq("id", id);
    if (error) return setAdminNotice(error.message);
    await loadRegistrations();
    setAdminNotice("Starter erfolgreich gelöscht.");
  }
  async function quickSetStatus(id, status) {
    const supabase = await getSupabaseClient();
    if (!supabase) {
      setRegistrations((prev) => prev.map((item) => (item.id === id ? { ...item, status } : item)));
      setAdminNotice(`Demo-Modus: Status auf ${status} gesetzt.`);
      return;
    }
    const { error } = await supabase.from("registrations").update({ status }).eq("id", id);
    if (error) return setAdminNotice(error.message);
    await loadRegistrations();
    setAdminNotice(`Status auf ${status} gesetzt.`);
  }
  ...
  const filteredRegistrations = useMemo(() => {
    return sortByKart(registrations.filter((entry) => {
      ...
      const matchesStatus = statusFilter === "Alle Status" || entry.status === statusFilter;
      const query = search.toLowerCase();
      const matchesSearch = !query || `${entry.firstName} ${entry.lastName}`.toLowerCase().includes(query) || entry.teamName.toLowerCase().includes(query) || entry.kartNumber.toLowerCase().includes(query);
      return matchesRace && matchesClass && matchesStatus && matchesSearch;
    }));
  }, [registrations, selectedRace, selectedClass, statusFilter, search]);
        ...
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="hero-grid">
          <div className="card">
            <div className="badges">
              ...
              <span className="pill gold">Live App + Admin Pro</span>
            ...
            <div className="hero-content">
              ...
              <div>
                ...
                <p className="muted">Komplette Live-Version mit Registrierung, Dashboard, Admin Pro, Filtersuche und CSV-Export.</p>
              </div>
            </div>
          </div>
          <div className="card">
            ...
            <div className="stack">
              ...
              <button onClick={loadRegistrations} className="rowbtn"><span><RefreshCw size={16} color="#facc15" /> Daten neu laden</span><span className="muted">{isLoading ? "lädt..." : "jetzt"}</span></button>
        ...
        {tab === "registration" && (
          <div className="form-wrap">
            ...
            <div className="card">
              ...
              <div className="stack">{raceStats.map((item) => <div key={item.race} className="rowbox"><div className="small">{item.race}</div><span className="pill red">{item.count}</span></div>)}</div>
            </div>
          </div>
        )}
        ...
        {tab === "dashboard" && (
          <>
            <div className="stats-grid">
              {[{ label: "Starter gesamt", value: stats.total, icon: Users, color: "#ef4444" },{ label: "Bestätigt", value: stats.confirmed, icon: Trophy, color: "#facc15" },{ label: "Offen", value: stats.open, icon: Gauge, color: "#e4e4e7" },{ label: "Aktive Klassen", value: stats.classesCount, icon: BarChart3, color: "#f87171" }].map((item) => <div key={item.label} className="card stat"><div><div className="muted small">{item.label}</div><div className="big">{item.value}</div></div><item.icon size={24} color={item.color} /></div>)}
            </div>
            <div className="main-grid">
              ...
              <div className="card">
                <div className="toolbar"><div><div className="section-title"><Filter size={18} color="#facc15" /> Starterübersicht</div><p className="muted small">Filtern, durchsuchen und als Datei exportieren.</p></div><button onClick={() => exportToCsv(filteredRegistrations)} className="btn redbtn"><Download size={16} /> Export CSV</button></div>
        ...
        {tab === "admin" && (!adminLoggedIn ? (
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="card admin-login-card">
            ...
            <form onSubmit={handleAdminLogin} className="admin-form">
              <label><span className="small">E-Mail / Benutzer</span><input value={adminEmail} onChange={(e) => setAdminEmail(e.target.value)} placeholder="admin@dkc.de" /></label>
              <label><span className="small">Passwort</span><input type="password" value={adminPassword} onChange={(e) => setAdminPassword(e.target.value)} placeholder="Passwort" /></label>
          ...
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
            <div className="rowbox"><div><div className="section-title"><Users size={18} color="#facc15" /> Admin Pro aktiv</div><p className="muted small">Bearbeiten, löschen, Status ändern und exportieren.</p></div><button onClick={handleAdminLogout} className="btn darkbtn"><LogOut size={16} /> Logout</button></div>
            <div className="card">
              <div className="toolbar"><div><div className="section-title"><Filter size={18} color="#facc15" /> Starterverwaltung</div><p className="muted small">Suche, Filter, Status-Quickactions, Bearbeiten und Löschen.</p></div><div className="toolbar-actions"><button onClick={loadRegistrations} className="btn darkbtn"><RefreshCw size={16} /> Neu laden</button><button onClick={() => exportToCsv(filteredRegistrations)} className="btn redbtn"><Download size={16} /> CSV Export</button></div></div>
              <div className="filter-grid">
                <label><span className="small">Rennen</span><select value={selectedRace} onChange={(e) => setSelectedRace(e.target.value)}><option>Alle Rennen</option>{races.map((race) => <option key={race}>{race}</option>)}</select></label>
                <label><span className="small">Klasse</span><select value={selectedClass} onChange={(e) => setSelectedClass(e.target.value)}><option>Alle Klassen</option>{classes.map((item) => <option key={item}>{item}</option>)}</select></label>
                <label><span className="small">Status</span><select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)}><option>Alle Status</option>{statusOptions.map((item) => <option key={item}>{item}</option>)}</select></label>
                  ...
                  return <div key={entry.id} className="entry-card">
                    {!editing ? <>
                      ...
                      <div className="action-row">
                        ...
                        <button onClick={() => quickSetStatus(entry.id, "Bestätigt")} className="btn greenbtn smallbtn"><BadgeCheck size={16} /> Bestätigt</button>
                        <button onClick={() => quickSetStatus(entry.id, "Bezahlt")} className="btn bluebtn smallbtn">Bezahlt</button>
                      </div>
